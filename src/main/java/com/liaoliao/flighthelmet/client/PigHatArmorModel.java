...
final class PigHatArmorModel extends Model {
    ...
    @Override
    public void renderToBuffer(PoseStack poses, VertexConsumer vertices, int light, int overlay, float red, float green, float blue, float alpha) {
        ...
        for (float[] quad : PigHatGeometry.QUADS) {
            for (int vertex = 0; vertex < 4; vertex++) {
                ...
                vertices.vertex(pose.pose(), quad[offset], quad[offset + 1], quad[offset + 2])
                        .color(red, green, blue, alpha).uv(quad[offset + 3], quad[offset + 4])
                        .overlayCoords(overlay).uv2(light)
                        .normal(pose.normal(), quad[0], quad[1], quad[2]).endVertex();
            }
        }
        poses.popPose();
    }
